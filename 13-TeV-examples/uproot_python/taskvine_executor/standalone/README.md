...
This runs the ATLAS Open Data H→γγ analysis with TaskVine and a separately started worker. The analysis is adapted from [`HyyAnalysis.ipynb`](../../HyyAnalysis.ipynb), and the download uses the same `atlasopenmagic`/`fsspec` cache setup as the notebook.

Run the commands below from this `standalone/` directory so the relative `data/` and output paths resolve here.
...
If an older checkout already has the files in `../data/`, reuse them with `python hyy_taskvine.py --data-dir ../data --port 9123` instead of downloading again.
